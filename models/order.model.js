...
export async function getOrderById(id) {
  const result = await pool.query("SELECT * FROM food_order WHERE order_id = $1", [id]);
  return result.rows[0];
}

export async function createOrder(customer_id, restaurant_id, status, order_date) {
  try {
...
export async function updateOrderById(id, customer_id, restaurant_id, status, order_date) {
